...
func TestURIFromPath1(t *testing.T) {
	type args struct {
		schema string
		path   string
	}
	tests := []struct {
		...
		{
			name: "windows file remote",
			args: args{
				schema: "file",
				path:   `\\server\share\path\to\file.txt`,
			},
			want: DocumentURI(`file://server/share/path/to/file.txt`),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := URIFromPath(tt.args.schema, tt.args.path); got != tt.want {
